...
try {
    ...
    if (location.match('https://mydramalist.com/profile/')) {
        ...
        if (username.classList.contains('md-up')) {
            console.log('[MDL+] class already fixed');
        }
...
// fast redirect & remove tracking
let streamingServices = document.querySelectorAll('#content > div > div.container-fluid.title-container > div > div.col-lg-8.col-md-8.col-rightx > div:nth-child(3) > div.box-body.wts a');
Array.from(streamingServices).forEach(function(anchor) {
...
// always open dramas in the same tab
var tableLinks = document.querySelectorAll('table a.title');
tableLinks.forEach(function(link) {
    link.removeAttribute('target');
    console.log('[MDL+] removed open in new tab for ' + link.getAttribute('href'));
});